...
/// ולמרות המחשבה הראשונה, גם אין לזה תיקון בזמן בנייה (למשל
/// `editbin /subsystem:windows`): נסינו, וה-exe קורס בעלייה (exit code
/// 255, בלי אף שורת לוג) בכל הרצה בלי stdio מחובר בכלל — כלומר בדיוק
/// בלחיצה כפולה, ב-Registry Run, וב-Task Scheduler. אז זה חלון קונסולה
/// אמיתי שאנחנו מסתירים בעצמנו מיד בעלייה — הפתרון היחיד שאינו דורש
/// קובץ עזר (VBS) או הרשאות מנהל, וגם לא מסתמך על handles שלא קיימים.
///
/// `bin/chavruta_companion.dart` קורא לפונקציה הזו כברירת מחדל, בלי צורך
/// בדגל `--hidden`, אלא אם המשתמש מבקש `--console` לצפייה חיה בפיתוח —
/// אז נשאר הבזק קצר של החלון עד שהקוד מספיק להסתיר אותו.
///
