...
/// **כולה**.
///
/// **זהו גבול על מספר הפריטים בלבד, והוא אינו מספיק לבדו.** מדידה עם
/// שמות ספרים אמיתיים ("שולחן ערוך אורח חיים", "שו״ת אגרות משה") נתנה
/// למנה של שנים-עשר בין 1265 ל-1985 בייט — כלומר מעל ה-MTU, ובניגוד
/// למה שנכתב כאן קודם. אורך שם הספר אינו חסום כלל, ולכן הצד השני קובע
/// בפועל את גודל ההודעה שלנו. החלוקה בפועל היא **לפי בייטים**, ראו
/// [maxDeskPayloadBytes]; הגבול הזה נשאר כמות שהוא כדי שמנה מגרסה
/// קודמת, ששולחת שנים-עשר, תמשיך להתקבל.
///
/// הפיצול חינם כאן משום שמיזוג השולחן הוא **לפי פריט**: לכל ספר יש
/// חותמת משלו, ולכן אין סדר בין מנות ואין תלות ביניהן. מנה שאבדה חוזרת
/// בשידור המלא התקופתי (ראו `deskResyncInterval`).
const int maxTabsPerMessage = 12;

/// תקציב הבייטים למנת שולחן אחת, כולל המעטפת והחתימה.
///
/// שמרני ביחס ל-MTU של 1500 פחות 28 בייט של כותרות IP/UDP: קישור
/// בלוטות' עובר גשרים ומנהרות שמוסיפים משלהם, וההפסד מפיצול הוא ההודעה
/// כולה. מנה של פריט אחד נשלחת גם אם היא חורגת — אין דרך לפצל פריט.
const int maxDeskPayloadBytes = 1200;
...
/// גבול אורך על מחרוזת שנוסעת על החוט: שם ספר, תיאור מיקום, ומזהה
/// המכשיר שקבע פריט שולחן.
///
/// **בלי הגבול הזה מחרוזת אחת חריגה סותמת את הקישור לגמרי, ובשקט.**
/// `RawDatagramSocket.send` אינו זורק על datagram שגדול מ-65507 בייט —
/// הוא מחזיר `0` (אומת בוינדוס), ו-`LanTransport.send` מפרש `0` כסוקט
/// מת: הוא זורק את סוקט השידור של הכרטיס, ואחרי כמה שידורים כאלה גם את
/// סוקט הקליטה, ומכריז בתוסף "המתאם אינו מאזין לקישור". כלומר שם ספר
/// אחד באורך חריג — מדיווח שולחן שגוי, או מחברותא בגרסה פגומה — נועל
/// את הסנכרון בלופ שנראה כתקלת רשת ואינו כזה.
///
/// 512 בייט הם יותר מכפליים משם הספר הארוך ביותר שאוצריא יכולה להחזיק
/// (רכיב נתיב בוינדוס חסום ב-255 תווים), ולכן שולח תקין אינו פוגש בגבול.
const int maxWireTextBytes = 512;

/// האם [value] ארוכה מ-[maxWireTextBytes] בייטים.
///
/// אורך ה-UTF-16 לעולם אינו גדול מאורך ה-UTF-8, ולכן הוא שער זול שפוסל
/// מחרוזת ענקית בלי לקדד אותה כולה רק כדי למדוד אותה.
bool wireTextTooLong(String value) =>
    value.length > maxWireTextBytes ||
    utf8.encode(value).length > maxWireTextBytes;
...
class SyncLocation {
  ...
  static SyncLocation? fromJson(Object? raw) {
    ...
    final ref = raw['ref'];
    // **אורך אינו נבדק כאן על אף שדה, ובכוונה גמורה.**
    //
    // [SyncMessage.decode] חותם מחדש על ההודעה **המשוחזרת** ומשווה
    // לחתימה שעל החוט. לכן כל שדה שהשחזור משמיט, חותך או מנרמל משנה את
    // הצורה הקנונית ומפיל את החתימה — כלומר בדיקת אורך כאן אינה "מסננת
    // שדה", אלא **דוחה בשקט את ההודעה כולה**. אומת בשתי בדיקות: הודעת
    // מיקום תקינה בת 997 בייט עם תיאור בן 800 בייט, והודעת נוכחות עם שם
    // ספר ארוך — שתיהן נדחו במלואן. בנוכחות המחיר גדול במיוחד: החברותא
    // נעלמת מרשימת המחוברים ונראית כמי שאינה מחוברת בכלל.
    //
    // וגרוע מכך, הדחייה נספרת ב-`datagramsRejected`, שסולם האבחון קורא
    // כ"הקוד אינו זהה" — ושולח את המשתמש לתקן את קוד החדר, שהוא תקין.
    //
    // הגבול נאכף בצד השולח בלבד, ב-`/publish`, וזה המקום היחיד שבו הוא
    // מגן על משהו: המיקום המקומי הוא שמשודר שוב בכל הודעת נוכחות.
    return SyncLocation(
      bookId: bookId,
      index: index,
...
class DeskEntry {
  ...
  static DeskEntry? fromJson(Object? raw) {
    ...
    // **אורך אינו נבדק כאן, מאותה סיבה שב-[SyncLocation.fromJson] — וכאן
    // הנפילה שקטה אף יותר.** פריט שנפסל נעלם מ-`entries`, ומערך `desk`
    // המשוחזר קצר ממה שנחתם: **כל** המנה נדחית, ולא רק הפריט החריג. אם
    // הפריט החריג הוא היחיד במנה, `desk` נעלם מהצורה הקנונית לגמרי.
    // כלומר חברותא שמריצה גרסה בלי הגבול — או ספר ששמו ארוך מהצפוי —
    // הייתה מאבדת את סנכרון השולחן כולו, והאבחון היה מאשים את קוד החדר.
    //
    // הגבול על מה ש**אנחנו** שולחים נאכף ב-`LocalApi._localEntries`, שם
    // הפריטים נולדים; פסילה שם היא פסילה של פריט לפני שנחתם עליו דבר,
    // ולכן היא אינה מפילה שום הודעה.
    final index = raw['i'];
    final open = raw['o'];
    return DeskEntry(
      bookId: bookId,
...
class SyncMessage {
  ...
  static SyncMessage? decode(
    ...
    //
    // **הגבול חל על הפריטים שאומתו, ולא על מערך החוט הגולמי.** `rawDesk`
    // אינו מכוסה בחתימה במלואו: [DeskEntry.listFromJson] זורק פריטים
    // פסולים בשקט, ולכן ריפוד ההודעה בפריטים פסולים אינו משנה את הצורה
    // הקנונית ואינו שובר את החתימה — אך כן מגדיל את `rawDesk.length`.
    // כלומר מי שקלט הודעה תקינה ושידר אותה מחדש עם ריפוד היה מפיל אותה
    // כאן. `entries` הוא תת-קבוצה של `rawDesk`, ולכן שולח תקין עובר את
    // הבדיקה בדיוק כמו קודם.
    if (message.entries.length > maxTabsPerMessage) return null;
