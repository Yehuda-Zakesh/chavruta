...
class LanTransport {
  ...
  /// כשל מניית הכרטיסים האחרון שדווח ליומן, לדה-דופליקציה בלבד. ראו
  /// [_routes] — הוא **אינו** מוצג בתוסף.
  String? _lastEnumerationError;
  ...
  Future<List<BroadcastRoute>> _routes() async {
    ...
    try {
      ...
      // רק על שינוי מצב, בדיוק כמו ב-[_doBind]. המנייה רצה בכל שידור —
      // פעימת נוכחות כל 20 שניות, ועוד אחת בכל מעבר דף — ולכן כשל
      // מתמשך היה כותב את אותה שורה עצמה עד שהרשת חוזרת, ומדחיק מהיומן
      // את מה שבאמת צריך לקרוא בו.
      //
      // **שדה נפרד ולא [_lastError], ובכוונה.** זה שמוצג בתוסף כתקלת
      // רשת, והוא מתאפס רק בקימה מחדש של הסוקט — כלומר מנייה שנכשלה
      // לרגע בזמן שהסוקט חי ונשאר חי הייתה נועצת שם שגיאה שאינה נמחקת
      // לעולם, והתוסף היה מציג תקלה על חיבור עובד.
      final reason = 'לא ניתן לרשום כרטיסי רשת: $e';
      if (_lastEnumerationError != reason) {
        _lastEnumerationError = reason;
        onLog?.call(reason);
      }
