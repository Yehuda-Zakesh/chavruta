...
(function () {
  ...
  /** החדר שההמתנה הנוכחית נמדדת בו. ראו [trackWaiting]. */
  let waitingRoom = '';

  function trackWaiting(waiting, room) {
    // **החלפת קוד מתחילה המתנה חדשה.** בלי זה `paired` נשאר אמת
    // והמחוברים מתרוקנים, ולכן המונה ממשיך מהחדר הקודם — ואבחנה מוצגת
    // בשנייה הראשונה של החדר החדש, במקום אחרי 45 שניות.
    if (room !== waitingRoom) {
      waitingRoom = room;
      waitingSince = 0;
    }
  ...
  function diagnoseEmptyWait(state) {
    ...
    // **קודם כול: האם בכלל שידרנו?** כל שאר האבחנות נשענות על כך
    // שהשידור שלנו חוזר אלינו, וההנחה הזאת תקפה רק אם יצא שידור. במחשב
    // בלי ראוטר, שהבלוטות' שלו עוד לא חובר, אין אף כרטיס שניתן לשדר
    // דרכו — ואז "אפס קליטה" אינו חומת האש אלא היעדר קישור. בלי השער
    // הזה הלשונית שלחה את המשתמש לחומת האש בזמן שהבאנר שמתחתיה אמר
    // נכונה "אין כאן קישור ישיר פעיל": שתי אבחנות סותרות, והבולטת
    // מביניהן היא השגויה.
    if (state.lanBound === false || !hasAnySendingLink(state.links)) {
      return 'המתאם אינו מצליח לשדר דרך שום כרטיס רשת, ולכן אין קישור ' +
        'בין המחשבים. ב-Windows חיבור הבלוטות\' אינו קם מעצמו — חברו אותו ' +
        'שוב (קליק ימני על סמל הבלוטות\' ← הצטרפות לרשת אישית).';
    }
    ...
    // **פער שעונים קודם לקוד.** הודעה שנדחתה על זמן כבר עברה אימות
    // חתימה, כלומר הקוד בשני הצדדים **כן** זהה — ואבחנה שאומרת "הקוד
    // אינו זהה" שולחת לתקן בדיוק את מה שתקין. המתאם מונה את שתי
    // הדחיות בנפרד; מתאם מגרסה קודמת אינו מדווח `datagramsClockRejected`
    // ואז נשען על `clockSkewMinutes` כפי שהיה.
    const clockRejected = state.datagramsClockRejected;
    if (
      (typeof clockRejected === 'number' && clockRejected > 0) ||
      typeof state.clockSkewMinutes === 'number'
    ) {
      return 'הודעות מהחברותא כן מגיעות והקוד זהה, אך הן נדחות מפני ' +
        'שהשעונים של שני המחשבים רחוקים זה מזה. תקנו את השעה במחשב שסוטה.';
    }
  ...
  /** האם יש כרטיס אחד לפחות שיוצא ממנו שידור בפועל. */
  function hasAnySendingLink(links) {
    if (!Array.isArray(links)) return true; // מתאם ישן שאינו מדווח — לא מניחים
    if (links.length === 0) return false;
    return links.some(function (link) {
      return link.sending !== false;
    });
  }
  ...
  function renderState(state) {
    ...
    trackWaiting(paired && peers.length === 0, paired ? activeRoom : '');
        ...
        '<code>' + escapeHtml(describeLinks(state.links)) + '</code><br>' +
          (hasDirectLink(state.links)
            ...
            : 'אין כאן קישור ישיר פעיל. חיבור בלוטות\' בין שני מחשבים נותן ' +
              'כתובת <code>169.254</code> שמשדרת, וזו דרך החיבור המיועדת. ' +
              'ב-Windows החיבור אינו קם מעצמו: לחצו ימני על סמל הבלוטות\' ← ' +
              '<b>הצטרפות לרשת אישית</b>, ובחרו את המחשב השני ← <b>התחבר באמצעות ' +
              'נקודת גישה</b>.<br>' +
              // הנוסח הקודם קבע ש"בלעדיה אין דרך להגיע לחברותא", וזה אינו
              // נכון: כל קישור IPv4 בין שני המחשבים עובד. שני מחשבים על
              // אותו Wi-Fi קיבלו כאן הוראה להקים בלוטות' לחינם.
              'אם שני המחשבים על אותה רשת (Wi-Fi משותף, נקודה חמה או כבל) — ' +
              'גם זה עובד, ואז השורה למעלה צריכה להיות זהה בשני המחשבים.')
      );
    } else {
      clearBanner('links');
    }
