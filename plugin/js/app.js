...
(function () {
  ...
  function diagnoseEmptyWait(state) {
    ...
    if (typeof received !== 'number') {
      return 'עדיין אין אף אחד. בדקו ששני המחשבים על אותה רשת, שהקוד זהה בדיוק, ' +
        'ושמתאם החברותא רץ גם בצד השני.';
    }
    if (received === 0) {
      return 'המתאם אינו שומע כלום מהרשת — גם לא את השידור של עצמו. ' +
    ...
    if (state.datagramsFromOthers === 0) {
      return 'השידור שלכם יוצא וחוזר, אך שום הודעה ממחשב אחר לא הגיעה. ' +
        'כלומר אין קישור בין שני המחשבים, או שהמתאם בצד השני אינו רץ.';
    }
  ...
  /** תיאור הקישורים שהמתאם משדר דרכם. השוואה בין שני מחשבים חושפת מיד
   *  אם הם על אותה רשת, וזה הדבר הראשון שצריך לבדוק. */
  function describeLinks(links) {
    if (!Array.isArray(links) || links.length === 0) return 'אין כרטיס רשת פעיל';
    return links
      .map(function (link) {
        return link.broadcast + ' (מ-' + link.local + ')';
      })
      .join(', ');
  }
    ...
    if (paired && peers.length === 0 && waitingTooLong()) {
      banner(
        ...
        '<code>' + escapeHtml(describeLinks(state.links)) + '</code><br>' +
          'השוו את השורה הזאת לזו שבמחשב השני. אם היא שונה — הם אינם על אותה ' +
          'רשת, וזו הבעיה. כתובת <code>169.254</code> לבדה פירושה מחשב שלא מצא ' +
          'רשת בכלל.'
      );
    } else {
      clearBanner('links');
